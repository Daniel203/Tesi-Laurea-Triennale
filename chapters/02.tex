...
variabile e produce un valore di lunghezza fissa. L'output generato è
deterministico, cioè che per una specifica stringa di input, l'hash code sarà
sempre lo stesso. Inoltre, la funzione è unidirezionale, il che significa che è
...
Il codice di output non è univoco perchè due input possono dare come risultato
lo stesso output, tuttavia la probabilità di questo evento, chiamato
\textit{collisione}, è molto rara data la grandezza dello spazio delle
soluzioni. Basti considerare che \textit{Bitcoin}, che utilizza l'algoritmo
SHA-256, ha $2^{256}$ possibili output, un numero talmente grande che rende
molto remota la possibilità di una collisione. Per dare un'idea di quanto
grande sia questo numero, si consideri che il numero di atomi nell'universo è
stimato essere $10^{80}$ mentre il numero di possibili output è circa
$10^{76}$, quindi solamente 4 ordini di grandezza inferiore.
